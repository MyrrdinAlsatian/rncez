...
function getComponentByType(args, cliConfigFile) {
  ...
  ${chalk.bold("rncez-config-cli.json")} config file under the ${chalk.bold(
            "component"
          )} object.
              `
        )
...
function componentTestTemplateGenerator({ cliConfigFile, cmd, componentName }) {
  const { customTemplates } = cliConfigFile.component[cmd.type];
  const { testFolder, testLibrary, usesTypeScript } = cliConfigFile;
  let template = null;
  let filename = null;
  ...
  if (customTemplates && customTemplates.test) {
    ...
    template = customTemplate;
    filename = customTemplateFilename;
    testPath = testFolder !== "None" ? "../TemplateName" : "./TemplateName";

    template = template.replace(`'TemplateTestName'`, `'${testPath}'`);
  } else {
    ...
    if (testLibrary === "Enzyme") {
      ...
      template = componentTestTestingLibraryTemplate.replace(
        /#|templateName/g,
  ...
  return {
    componentPath: `${cmd.path}/${componentName}/${testFolder}/${filename}`,
    filename,
    template,
  };
}
...
function componentStoryTemplateGenerator({
  ...
  return {
    componentPath: `${cmd.path}/${componentName}/story/${filename}`,
    filename,
    template,
  };
}
...
function componentJsonLazyPackageGenerator({
  cmd,
  componentName,
  cliConfigFile,
}) {
...
function componentSSRGenerator({ cmd, componentName, cliConfigFile }) {
  const { customTemplates } = cliConfigFile.component[cmd.type];
  let template = null;
  let filename = null;
...
function componentSSGGenerator({ cmd, componentName, cliConfigFile }) {
  const { customTemplates } = cliConfigFile.component[cmd.type];
  let template = null;
  let filename = null;
  ...
  return {
    componentPath: `${cmd.path}/pages/${filename}`,
    filename,
    template,
  };
}

function componentSSGReduxGenerator({ cmd, componentName, cliConfigFile }) {
  const { customTemplates } = cliConfigFile.component[cmd.type];
  let template = null;
  let filename = null;
...
function componentSSRReduxGenerator({ cmd, componentName, cliConfigFile }) {
  const { customTemplates } = cliConfigFile.component[cmd.type];
  let template = null;
  let filename = null;
...
const componentTemplateGeneratorMap = {
  ...
  [buildInComponentFileTypes.JSON]: componentJsonPackageGenerator,
  [buildInComponentFileTypes.JSONLAZY]: componentJsonLazyPackageGenerator,
  [buildInComponentFileTypes.SSR]: componentSSRGenerator,
  [buildInComponentFileTypes.SSG]: componentSSGGenerator,
  [buildInComponentFileTypes.SSGRedux]: componentSSGReduxGenerator,
  [buildInComponentFileTypes.SSGRedux]: componentSSRReduxGenerator,
};
